...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("查看未进入部门时，解散部门按钮是否会出现")
        text_list = Common.get_text_by_class_name(self, ClassName.ivu_btn_warning, "button")
        self.assertFalse(Common.check_text_in_list(self, text_list, "解散部门"))

        logger.info("查看创建子部门按钮是否出现")
        Common.touch_text_by_class_name(self, ClassName.name, "zs_test", "span")
        text_list = Common.get_text_by_class_name(self, ClassName.ivu_btn_warning, "button")
        self.assertTrue(Common.check_text_in_list(self, text_list, "解散部门"))
