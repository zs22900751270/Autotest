...
class MyTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        Common.delete_department_by_name(self, "new_name")
        Common.quit(self)

    def test_step(self):
        u"""pc-重命名部门时，取消/确定按钮验证"""
        logger.info("登录后端")
        ...
        Common.send_text_by_class_name_and_palceholder(self, ClassName.ivu_input, "请输入部门名称", "new_name")
        Common.touch_text_by_class_name(self, ClassName.ivu_btn_text_large, "取消", "button")
        name_ele_list = Common.get_class_name_elements_by_element(self, department_name, ClassName.name)
        name_list = Common.get_text_by_elements(self, name_ele_list)
        self.assertFalse(Common.check_text_in_list(self, name_list, "new_name"))

        logger.info("点击重命名,选择确定")
        Common.touch_text_by_class_name(self, ClassName.ivu_reload_success, "重命名", "button")
        Common.send_text_by_class_name_and_palceholder(self, ClassName.ivu_input, "请输入部门名称", "new_name")
        Common.touch_text_by_class_name(self, ClassName.ivu_btn_large, "确定", "button")
        name_ele_list = Common.get_class_name_elements_by_element(self, department_name, ClassName.name)
        name_list = Common.get_text_by_elements(self, name_ele_list)
        self.assertTrue(Common.check_text_in_list(self, name_list, "new_name"))
