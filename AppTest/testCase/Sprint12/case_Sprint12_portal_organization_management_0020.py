...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        del_btn = Common.get_results_by_class_name_blank(self, "button", ClassName.ivu_btn_error_small)
        for name in con_list:
            if Common.get_realname_by_phone(self, Content.spare_count) in name:
                Common.touch_by_element(self, del_btn[con_list.index(name)])
        Common.touch_text_by_class_name(self, ClassName.ivu_btn_large, "确定", "button")
