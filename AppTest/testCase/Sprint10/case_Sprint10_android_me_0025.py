...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        u"""安卓-设置-修改密码页面-输入新密码和确认新密码且和老密码一致"""
        ...
        BaseOperate.sendTextById(self, PhoneControl.id_password_tel_num, Content.login_password)
        BaseOperate.sendTextById(self, PhoneControl.id_password_verify_code_edit, Content.login_password)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_password_next_btn, t=0)

        logger.info("点击是否出现错误提示)")
        self.assertTrue(BaseOperate.find_toast(self, "新密码和旧密码不能相同"))
