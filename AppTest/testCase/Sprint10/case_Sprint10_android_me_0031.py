...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        iden_code = BaseOperate.get_identifying_code(self, Content.no_register_count)
        BaseOperate.sendTextById(self, PhoneControl.id_verify_code_edit, iden_code)
        for i in range(11):
            BaseOperate.wait(self, 30)
            BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_title_tv)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_next_btn, t=0)
        self.assertTrue(BaseOperate.find_toast(self, "验证码已失效,请重新获取"))
