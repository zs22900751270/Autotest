...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        restule = BaseOperate.check_text_in_list(self, name_list, "group_name")
        self.assertTrue(restule)

        logger.info("修改分组名称")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_group_name)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)
        BaseOperate.sendTextById(self, PhoneControl.id_group_name, "name_group_new")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)

        logger.info("判断是否成功修改分组名称")
        group_name = BaseOperate.get_text_list_by_id(self, PhoneControl.id_group_name)
        check_text = BaseOperate.check_text_in_list(self, group_name, "name_group_new")
