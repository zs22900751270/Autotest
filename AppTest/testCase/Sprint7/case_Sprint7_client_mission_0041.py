...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("获取任务开始时间")
        now = datetime.datetime.now()
        format_start_time = now + datetime.timedelta(hours=-1)
        format_end_time =  now + datetime.timedelta(hours=-2)
        start_time = datetime.datetime.strftime(format_start_time, "%Y-%m-%d %H:%M:%S")
        end_time = datetime.datetime.strftime(format_end_time, "%Y-%m-%d %H:%M:%S")
