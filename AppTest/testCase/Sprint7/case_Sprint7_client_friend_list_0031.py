...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("获取当前好友数量")
        num = Common.get_text_by_class_name(self, ClassName.name, "span")

        logger.info("点击接受好友请求")
        Common.accept_or_not_friend_request(self, Content.register_count)

        logger.info("查看是否已接受好友请求")
        check_res = Common.get_friend_request_result(self, Content.register_count)
        self.assertTrue(check_res)

        logger.info("获取当前好友数量")
        num1 = Common.get_text_by_class_name(self, ClassName.name, "span")

        logger.info("判断好友数量是否增加")
        self.assertEqual(len(num)+1, len(num1))
