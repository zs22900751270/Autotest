...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        self.assertTrue(BaseOperate.check_text_in_list(self, name_list, "group_name"))
        group_name2 = BaseOperate.get_text_by_id(self, PhoneControl.id_group_name)[-2:-1]
        self.assertEqual(group_name2, "2")

        logger.info("删除分组成员")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_group_name)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_remove)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)

        logger.info("判断是否删除成功")
        group_name1 = BaseOperate.get_text_by_id(self, PhoneControl.id_group_name)[-2:-1]
        self.assertEqual(group_name1, "1")
