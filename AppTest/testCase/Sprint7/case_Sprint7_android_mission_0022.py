...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("判断是否进入抄送人界面")
        tit_name = BaseOperate.get_text_by_id(self, PhoneControl.id_toolbar_title_tv)
        self.assertEqual(tit_name, "人员选择")
        ...
        logger.info("判断是否选择一个成功")
        copy_num = BaseOperate.get_text_by_id(self, PhoneControl.id_copys_count)
        self.assertEqual(copy_num, "（1人）")

        logger.info("选择三个抄送人")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_copy_layout)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_contact)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_contact_layout, 1)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_contact_layout, 2)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_contact_layout, 3)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)

        logger.info("判断是否选择一个成功")
        copy_num = BaseOperate.get_text_by_id(self, PhoneControl.id_copys_count)
        self.assertEqual(copy_num, "（3人）")
