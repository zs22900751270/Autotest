...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("再添加好友界面搜索未注册的账号")
        Common.search_friend_by_name(self, Content.no_register_count)

        logger.info("判断搜索结果是否出现‘用户不存在’")
        search_res = Common.get_text_by_class_name(self, ClassName.tips, "div")
        check_res = Common.check_text_in_list(self, search_res, "用户不存在")
