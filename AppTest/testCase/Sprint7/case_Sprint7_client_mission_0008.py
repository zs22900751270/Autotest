...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("在编辑任务界面，点击‘任务’，可以返回任务界面")
        Common.touch_text_by_class_name(self, ClassName.ivu_breadcrumb_item_link, "任务", "a")

        logger.info("判断是否可以返回任务界面")
        result = Common.get_text_by_class_name(self, ClassName.ivu_breadcrumb_item_link, "span")[-1]
        self.assertEqual(result, "任务")

        logger.info("在编辑任务界面，点击‘服务’，可以返回服务界面")
        Common.open_mission_detail_by_name(self, "mission_content", Content.register_count)
        Common.touch_text_by_class_name(self, ClassName.ivu_breadcrumb_item_link, "服务", "a")

        logger.info("判断是否可以返回服务界面")
        result = Common.get_text_by_class_name(self, ClassName.ivu_menu_item_active_selected, "li")[0]
        self.assertEqual(result, "服务")
