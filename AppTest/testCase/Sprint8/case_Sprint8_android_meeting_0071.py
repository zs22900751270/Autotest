...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        s_time, e_time = BaseOperate.get_start_and_end_time(self, "minute", 12)
        ...
        logger.info("查看签到按钮是否存在")
        BaseOperate.touch_text_by_class_name(self, PhoneControl.class_name_TextView, "确认参加")
        res1 = BaseOperate.checkIfIdExist(self, PhoneControl.id_sign)
        self.assertFalse(res1)

        logger.info("修改会议时间在10分钟以内，查看签到按钮是否出现")
        s_time, e_time = BaseOperate.get_start_and_end_time(self, "minute", 9)
        BaseOperate.modify_meeting_time(self, s_time, e_time, "meeting_test", Content.register_count)
