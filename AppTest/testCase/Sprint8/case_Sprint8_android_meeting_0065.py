...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("判断是否进入取消会议界面")
        cancel_meeting = BaseOperate.get_text_by_id(self, PhoneControl.id_toolbar_title_tv)
        self.assertTrue(cancel_meeting, "输入不参加原因")

        logger.info("输入不参加原因点击保存，判断是否可以保存成功")
        ...
        logger.info("点击进入“不参加”界面查看，是否已标记为不参加")
        ...
        text_list1 = BaseOperate.get_text_list_by_id(self, PhoneControl.id_status)
        check_1 = BaseOperate.check_text_in_list(self, text_list1, "不参与")
        self.assertEqual(len(text_list1), 1)
