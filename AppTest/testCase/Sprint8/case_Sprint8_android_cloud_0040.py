...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        for i in range(1, 16):
            ...
            BaseOperate.sendTextById(self, PhoneControl.id_remark, "longlonglonglonglonglonglonglonglong%s" % i)
            ...
            self.assertTrue(BaseOperate.check_text_in_list(self, name_list, "longlonglonglonglonglonglonglonglong%s" % i))
            self.assertEqual(len(name_list), 1)
            if i != 1:
                self.assertTrue(BaseOperate.check_text_in_list(self, path_list, "longlonglonglonglonglonglonglonglong%s" % (i-1)))
            if i != 15:
                BaseOperate.touch_text_by_id(self, "longlonglonglonglonglonglonglonglong%s" % i, PhoneControl.id_name)
