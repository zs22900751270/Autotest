...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("获取当前时间与会议预期结束时间")
        e_time = BaseOperate.get_start_and_end_time(self, "minute", 0)[1]
        now_time = BaseOperate.get_text_by_id(self, PhoneControl.id_end_date)
        logger.info(now_time)
        logger.info(e_time)
        self.assertTrue(now_time < e_time)
