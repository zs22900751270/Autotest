...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        u"""数据标准字典查询框可以正常查询中文、英文、特殊字符的字段名称"""
        logger.info("web端登录")
        ...
        for i in range(5):
            ...
            if i <= 2:
                Common.send_text_by_class_name_and_palceholder(self, ClassName.ivu_input, "请输入字段名称", str_name[i])
            else:
        ...
        for j in str_name:
            ...
            Common.send_text_by_class_name_and_palceholder(self, ClassName.ivu_input, "字段名称", j+"\n")

            logger.info("判断是否能搜索到所创建的字典")
            result = Common.get_results_by_class_name_blank(self, "tr", ClassName.ivu_table_row)
