...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        self.assertTrue(BaseOperate.check_text_in_list(self, theme_list_all, "mail_theme_1"))
        self.assertTrue(BaseOperate.check_text_in_list(self, theme_list_all, "mail_theme_2"))
        self.assertTrue(BaseOperate.check_text_in_list(self, theme_list_all, "mail_theme_3"))

        logger.info("点击编辑")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)

        logger.info("判断是否进入编辑模式")
        ...
        self.assertFalse(BaseOperate.check_text_in_list(self, text_list, "编辑"))
        self.assertTrue(BaseOperate.checkIfIdExist(self, PhoneControl.id_select))

        logger.info("退出编辑模式")
        BaseOperate.touch_id_by_index(self, PhoneControl.id_toolbar_right_tv)
        text_list = BaseOperate.get_text_by_class_name(self, PhoneControl.class_name_TextView)
        self.assertTrue(BaseOperate.check_text_in_list(self, text_list, "编辑"))
        self.assertFalse(BaseOperate.check_text_in_list(self, text_list, "取消"))
        self.assertFalse(BaseOperate.checkIfIdExist(self, PhoneControl.id_select))
