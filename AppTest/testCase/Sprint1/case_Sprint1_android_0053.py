...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        # 重复输入密码（两次输入不同密码）
        BaseOperate.sendTextById(self, PhoneControl.forget_input_password_again, "qqqq1111")
        # 点击下一步
        BaseOperate.touch_id_by_index(self, PhoneControl.forget_input_password_next_button)
        # 进入登陆界面
        status = BaseOperate.get_text_by_id(self, PhoneControl.login_login_button)
        self.assertTrue(status == "登录")
        ...
        BaseOperate.touch_id_by_index(self, PhoneControl.forget_next_button)
        # 输入新密码
        BaseOperate.sendTextById(self, PhoneControl.forget_input_password, password)
        # 重复输入密码（两次输入不同密码）
        BaseOperate.sendTextById(self, PhoneControl.forget_input_password_again, password)
        # 点击下一步
        BaseOperate.touch_id_by_index(self, PhoneControl.forget_input_password_next_button)
        # 进入登陆界面
        status = BaseOperate.get_text_by_id(self, PhoneControl.login_login_button)
