...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        # 输入新密码空格
        BaseOperate.sendTextById(self, PhoneControl.forget_input_password, "       ")
        # 重复输入密码（两次输入不同密码）
        BaseOperate.sendTextById(self, PhoneControl.forget_input_password_again, "       ")
        starus = BaseOperate.get_text_by_id(self, PhoneControl.id_password_warning_tv)
        self.assertTrue(starus == "请输入数字字母组合")
