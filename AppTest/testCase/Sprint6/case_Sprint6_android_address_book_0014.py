...
class MyTestCase(unittest.TestCase):
    ...
    def test_step(self):
        ...
        logger.info("只会出现一个好友请求")
        count = BaseOperate.get_text_by_id(self, PhoneControl.id_new_count)
        ...
        BaseOperate.except_friend_requset_or_not(self, friend_name)
        ...
        logger.info("输入手机号码进行查找")
        BaseOperate.sendTextById(self, PhoneControl.id_search_edit, Content.register_count)
        BaseOperate.touch_search_by_id(self, PhoneControl.id_search_edit)

        logger.info("判断添加按钮是否存在")
        result = BaseOperate.checkIfIdExist(self, PhoneControl.id_add)
